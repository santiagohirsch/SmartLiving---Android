package com.example.myapplication

class MyIntent {
    companion object {
        const val SHOW_NOTIFICATION = "com.example.myapplication.SHOW_NOTIFICATION"
        const val SKIP_NOTIFICATION = "com.example.myapplication.SKIP_NOTIFICATION"
        const val DEVICE_ID = "com.example.myapplication.DEVICE_ID"
        const val EVENT = "com.example.myapplication.EVENT"
        const val ARGS = "com.example.myapplication.ARGS"
        const val PACKAGE = "com.example.myapplication"
    }
}
